...
**界面打不开怎么办**：图形界面用 wgpu 渲染，需要能用的显卡驱动（虚拟机上要开 3D 加速，否则可能起不来）。启动失败时会弹一个错误框，同时在程序同目录留下 `vzip-gui.log`，里面有详细原因；把它发出来即可。日志默认只记 info；想看得更细，先 `set VZIP_LOG=debug`（Linux/macOS 用 `export`）再运行。

Windows 上默认在 DX12 和 OpenGL 里挑（不碰 Vulkan），避开个别有问题的 Vulkan 驱动；想单独指定后端，`set WGPU_BACKEND=dx12`（或 `gl`、`vulkan`）再运行。
